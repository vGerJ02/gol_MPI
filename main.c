...
int main(int argc, char **argv) {
  ...
  // Receive the filled board and Endtime
  MPI_Bcast(board, sizeof(board_t), MPI_BYTE, 0, MPI_COMM_WORLD);
  MPI_Bcast(&EndTime, 1, MPI_INT, 0, MPI_COMM_WORLD);
  ...
  // Load balancing
  data_mpi_t data;
  basic_load_balance(&data, size, board->ROW_NUM, board->COL_NUM);
  ...
  while (quit == false && (EndTime < 0 || Iteration < EndTime)) {
    ...
    if (rank == 0) {
      if (Graphical_Mode) {
        ...
        // peeper.y); fflush(stdout); printf("Peeper size: (%d,
        ...
        if (window_width > maximum_width) {
          printf(
              "WARNING: Attempting to exceed max window size in x (win width: "
              "%d - win max width: %d).\n",
...
void basic_load_balance(data_mpi_t *data, int size, int rows, int cols) {
  data->rows_per_process = rows / size;
  data->remaining_rows = rows % size;

  data->sendcounts = (int *)malloc(size * sizeof(int));
  data->displs = (int *)malloc(size * sizeof(int));

  // Set the size of each subarray and the offset
  int offset = 0;
  for (int i = 0; i < size; i++) {
    data->sendcounts[i] = (i < data->remaining_rows)
                              ? (data->rows_per_process + 1) * cols
                              : data->rows_per_process * cols;
